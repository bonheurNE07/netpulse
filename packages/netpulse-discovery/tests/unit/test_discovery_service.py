...
async def test_discover_network_arp_success():
    ...
    with patch("netpulse.discovery.services.discovery.scan_arp") as mock_scan_arp:
...
async def test_discover_network_icmp_success():
    ...
    with patch("netpulse.discovery.services.discovery.scan_icmp") as mock_scan_icmp:
...
async def test_discover_network_icmp_with_arp_cache_resolution():
    ...
    with patch("netpulse.discovery.services.discovery.scan_icmp") as mock_scan_icmp, \
         patch("netpulse.discovery.services.mac_lookup.MacLookupService.parse_system_arp_table") as mock_arp:
...
async def test_discover_network_partial_failures():
    ...
    with patch("netpulse.discovery.services.discovery.scan_arp") as mock_scan_arp, \
         patch("netpulse.discovery.services.discovery.scan_icmp") as mock_scan_icmp:
